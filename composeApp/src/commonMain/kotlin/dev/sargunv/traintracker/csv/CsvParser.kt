...
class CsvParser(
    ...
    fun parseWithoutHeader(): Sequence<List<String>> = sequence {
        input.use {
            val (firstRecord, pos) = readRecord(0)
                ?: throw CsvParseException("Expected at least one record")
            var cursor = readEndOfLine(pos)?.newPos
            ...
            while (true) {
                ...
                if (record.size != numColumns) {
                    throw CsvParseException("Expected $numColumns columns, got ${record.size} in record $record")
                }
                cursor = readEndOfLine(newPos)?.newPos
    ...
    fun parseWithHeader(checkHeader: (header: List<String>) -> Boolean = { true }): Sequence<Map<String, String>> {
        ...
        return records.asSequence().map { record ->
