...
private fun Source.GeoJson.toNativeSource(id: String) =
  MLNShapeSource(
    identifier = id,
...
private fun MLNStyleLayer.applyFrom(layer: Layer) {
  layer.minZoom?.let { setMinimumZoomLevel(it) }
  layer.maxZoom?.let { setMaximumZoomLevel(it) }
}

private fun Layer.Type.Line.toNativeLayer(
...
private class MapViewDelegate(val getLatestOptions: () -> MapViewOptions) :
    ...
    getLatestOptions()
      ...
      .forEach { (layer, nativeLayer) ->
        when {
          ...
          layer.index != null ->
            didFinishLoadingStyle.insertLayer(layer = nativeLayer, atIndex = layer.index.toULong())
...
private fun MLNMapView.applyUiOptions(
