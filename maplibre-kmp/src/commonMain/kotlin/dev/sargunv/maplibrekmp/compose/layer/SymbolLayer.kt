 ...
 *
 * @param spacing
 *   Distance between two symbol anchors in dp.
 *
 ...
 *
 * @param iconHaloWidth
 *   Distance of halo to the icon outline. The unit is in dp only for SDF sprites that were created
 *   with a blur radius of 8, multiplied by the display density. I.e., the radius needs to be 16 for
 *   @2x sprites, etc.
 *
 *   Ignored if [iconImage] is not specified.
 *
 * @param iconHaloBlur
 *   Fade out the halo towards the outside, in dp.
 *
 *   Ignored if [iconImage] is not specified.
 *
 * @param iconSize
 ...
 *   Ignored if [textField] is not specified.
 *
 * @param textRotationAlignment
 *   In combination with [placement], determines the rotation behavior of the individual glyphs
 *   forming the text. See [TextRotationAlignment].
 *
 *   Ignored if [textField] is not specified.
 *
 ...
 *
 * @param textMaxWidth
 *   The maximum line width in ems for text wrapping.
 *
 *   Ignored if [textField] is not specified.
 *
 * @param textLineHeight
 *   Text leading value in ems for multi-line text.
 *
 *   Ignored if [textField] is not specified.
 *
 * @param textJustify
 ...
 *   Ignored if [textField] is not specified.
 *
 * @param textVariableAnchor
 *   To increase the chance of placing high-priority labels on the map, you can provide an array of
 *   [textAnchor] locations: the renderer will attempt to place the label at each location, in
 *   order, before moving onto the next label. Use [textJustify] = [TextJustify.Auto] to choose
 *   justification based on anchor position. To apply an offset, use the [textRadialOffset] or the
 ...
 *   Ignored if [textField] is not specified.
 *
 * @param textVariableAnchorOffset
 *   To increase the chance of placing high-priority labels on the map, you can provide an array of
 *   [SymbolAnchor] locations, each paired with an offset value. The renderer will attempt to place
 *   the label at each location, in order, before moving on to the next location+offset.
 *   Use [textJustify] = [TextJustify.Auto] to choose justification based on anchor position.
 *
 ...
 *   ```
 *   When the renderer chooses the top anchor, [0, 4] will be used for [textOffset]; the text will
 *   be shifted down by 4 ems. When the renderer chooses the left anchor, [3, 0] will be used for [textOffset]; the text will
 *   be shifted right by 3 ems. Etc.
 *
 *   Ignored if [textField] is not specified.
 *
 * @param textPadding
 ...
 *   See [SymbolOverlap]. Overrides [textAllowOverlap].
 *
 *   Ignored if [textField] is not specified.
 *
 *   **Note**: This property is not supported on native platforms, yet.
 *
 * @param textIgnorePlacement
 *   If true, other symbols can be visible even if they collide with the text.
 *
 *   Ignored if [textField] is not specified.
 *
 * @param textOptional
 *   If true, icons will display without their corresponding text when the text collides with other
 *   symbols and the icon does not.
 *
 ...
 *   Controls the frame of reference for [textTranslate]. See [TranslateAnchor].
 *
 *   Ignored if [textField] is not specified.
 * */
@Composable
...
public object SymbolSortOrder {
   ...
   *  `textAllowOverlap` is set to `true` or `iconIgnorePlacement` or `textIgnorePlacement`
   *  is `false`. */
